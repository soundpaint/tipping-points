...
\begin{frame}
  ...
  \item<2-> \color<2>{\clr} {\em Gegenkopplung} oder {\em negative} Rückkopplung
    ...
    \item<4-> \color<4>{\clr} Beispiele:
      \begin{itemize}
      \item<5-> \color<5>{\clr} Temperaturregler, Spannungsregler, Fliehkraftregler, …
      \item<6-> \color<6>{\clr} Noise Cancelling
      \item<7-> \color<7>{\clr} Schilddrüsenfunktion
      \item<8-> \color<8>{\clr} Angebot und Nachfrage
      \item<9-> \color<9>{\clr} Temperaturkompensation, Frequenzkompensation
      \item<10-> \color<10>{\clr} Autofokus (Flankendiskriminator)
      \end{itemize}
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}[t]
  ...
  \item<5-> \color<5>{\clr} Sättigung: Begrenzung der Ausgangsgröße
    ...
    \end{itemize}
    \item[]<9->{\color<9>{\clr} {\imply} Schwellwert in System mit
      zwei (oder mehr) Zuständen}
    \item[]<10->{\color<10>{\clr} {\imply} rascher Zustandswechsel bei
  ...
  \begin{itemize}
    \item<2-> \color<2>{\clr} (Kleine) Hysterese praktisch immer
      ...
      \item<4-> \color<4>{\clr} Durch Reibung, Wegstrecke,
        Abschmelzen, etc.
      \end{itemize}
    \item<5-> \color<5>{\clr} Größe der Hysterese abhängig von Mitkopplung
      \begin{itemize}
      \item<6-> \color<6>{\clr} Entscheidend für Irreversibilität
      \item<7-> \color<7>{\clr} Ggf. schwer zu bestimmen / abzuschätzen
      \end{itemize}
    \item<8-> \color<8>{\clr} Hysterese ggf. schon bei Zustandsübergang voll wirksam
    \item<9-> \color<9>{\clr} veschiedene Hystereseformen und -dynamiken möglich
    \item<10-> \color<10>{\clr} Modelle stets Vereinfachung der Realität
    \item<11-> \color<11>{\clr} Genauigkeit / Aussagekraft abhängig von Detaillierungsgrad
    \item<12-> \color<12>{\clr} Ggf. unerwartete Effekte als Fingerzeig für weitere Untersuchungen
  \end{itemize}
\end{frame}
...
\begin{frame}[t]
  ...
  zur Hysterese
  \begin{itemize}
    \item<2-> \color<2>{\clr} Z.B. Schneeflächen: Wie groß ist die Hysterese?
      \begin{itemize}
    ...
    \item<5-> \color<5>{\clr} Form der Hysterese?
      \begin{itemize}
        \item<6-> \color<6>{\clr} Parallelogramm?

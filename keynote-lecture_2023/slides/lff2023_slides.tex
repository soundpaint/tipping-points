...
\begin{frame}[t]
  \frametitle{Motivation \& Ziele II}
  \begin{itemize}
  \item<2-> \color<2>{\clr} Was ist eine {\em Rückkopplung?}
  \item<3-> \color<3>{\clr} Was ist ein {\em Kipppunkt} (engl. {\em
    tipping point})?
  \item<4-> \color<4>{\clr} Welche Beispiele gibt es?
  \item<5-> \color<5>{\clr} Was sind die grundsätzlichen Eigenschaften?
  \item<6-> \color<6>{\clr} Warum können die Auswirkungen {\em abrupt} sein?
  \item<7-> \color<7>{\clr} Warum können die Auswirkungen {\em
    irreversibel} sein?
  \item<8-> \color<8>{\clr} Welche weiteren Besonderheiten können
    auftreten?
  \end{itemize}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Rückkopplung I}
...
\begin{frame}
  ...
  \vspace{-3.0cm}\\
  \begin{itemize}
  \item<2-> \color<2>{\clr} {\em Gegenkopplung} oder {\em negative} Rückkopplung
    \begin{itemize}
    \item<3-> \color<3>{\clr} Abschwächende, dämpfende Wirkung
    \item<4-> \color<4>{\clr} Beispiele:
      \begin{itemize}
      \item<5-> \color<5>{\clr} Regelkreis / Thermostat:
      \item<6->[] \color<6>{\clr} Temperaturregler (Kühlschrank, Heizung)
      \item<7->[] \color<7>{\clr} Spannungsregler / Netzteil
      ...
      \item<12-> \color<12>{\clr} Temperaturkompensation, Frequenzkompensation
...
\begin{frame}[t]
  ...
  \item<2-> \color<2>{\clr} Mitkopplung
    \begin{itemize}
    \item[]<3-> \color<3>{\clr} {\imply} hohe Verstärkung
    \item[]<4-> \color<4>{\clr} {\imply} steile Flanke
    \end{itemize}
  \item<5-> \color<5>{\clr} Sättigung: Begrenzung der Ausgangsgröße
    \begin{itemize}
    \item<6-> \color<6>{\clr} Beispiel: Reflexion (Albedo) von Schnee
      auf dunklem Untergrund
      \begin{itemize}
      \item<7-> \color<7>{\clr} maximal bei 100\% Schneebedeckung
      \item<8-> \color<8>{\clr} minimal bei 0\% Schneebedeckung
      \end{itemize}
    \item<9-> \color<9>{\clr} Beispiel Wippe: Begrenzte Wegstrecke
    \end{itemize}
    ...
    \item[]<11->{\color<11>{\clr} {\imply} rascher Zustandswechsel bei
      Überschreiten eines Schwellwerts ({\em Kipppunkt})}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item<3-> \color<3>{\clr} Zustand 2 (Neigung zur anderen Seite,
      wärmeres Klima)
    \item<4-> \color<4>{\clr} Wechsel{\em bereich} Zustand 1 {\imply}
      2
    \item<5-> \color<5>{\clr} Wechsel{\em bereich} Zustand 2 {\imply}
      1
    \item<6-> \color<6>{\clr} $\leadsto$ besser {\em Kippelement} mit
...
\begin{frame}[t]
  \frametitle{Beispiel Klimakipppunkte} (nach Schellnhuber, Stand 2008)
  \supercite{Wikipedia20a,Maeder08}
  \begin{itemize}
...
\begin{frame}
  \frametitle{Beispiel mechanisches Modell: Wippe}
  \tikz[remember picture, overlay] {
...
\begin{frame}
  \frametitle{Computersimulation der Wippe}
  \tikz[remember picture, overlay] {
    \node[shift={(0.50cm, 0.50cm)}] at (current page.south west) (t1) {
      \begin{tikzpicture}[remember picture, overlay]
...
\begin{frame}
  \frametitle{Computersimulation der Wippe: Ergebnisse}
  \begin{itemize}
...
\begin{frame}
  \frametitle{Elektronisches Modell}
  \vspace{1.5cm} \tikz[remember picture, overlay] {
    \vspace{2.5cm} {\includegraphics[width=0.9\paperwidth,center]
      {../images/electronics-setup.jpg}} }\\
  \begin{itemize}
  \item<2-> \color<2>{\clr} Oszilloskop zur Visualisierung
  \item<3-> \color<3>{\clr} elektronisches Modell auf Breadboard
  \item<4-> \color<4>{\clr} Labornetzgerät, Spannunsversorgung 12V
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{Elektronisches Modell: Visualisierung}
  \begin{itemize}
    ...
    \item<4-> \color<4>{\clr} Bildwiederholung für stehendes Bild
      notwendig
    \item<5-> \color<5>{\clr} hierfür mit Oszillator Kipppelement
      periodisch durchwandern
    \item<6-> \color<6>{\clr} {\imply} Dreieckschwingung erzeugen für
      Durchlaufen der X-Achse
...
\begin{frame}
  \frametitle{Elektronisches Modell: Implementierung}
...
\begin{frame}
  \frametitle{Elektronisches Modell: Erzeugung Dreieckschwingung}
  \begin{itemize}
  ...
  \item<8-> \color<8>{\clr} Frequenz der Oszillation im hörbaren
    Bereich
  \item<9-> \color<9>{\clr} Kalibrierung der Linearität (Reduktion
    Klirrfaktor) ggf. auch akustisch möglich
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{Elektronisches Modell: Dreickschwingung für X-Achse}
  \begin{figure}
    \centering
    \label{fig:charging_voltage}
    {\includegraphics[width=0.5\paperwidth,center]
      {../images/charging-voltage.jpg}}
    \caption{Der Spannungsverlauf der Ladekurve am Kondensators des
...
\begin{frame}
  \frametitle{Elektronisches Modell: Hysterese bei Volldurchlauf}
  \begin{figure}
    \centering
...
\begin{frame}
  \frametitle{Elektronisches Modell: Kehrtwende im Hysteresebereich}
  \begin{figure}
    \centering
...
\begin{frame}
  \frametitle{Elektronisches Modell: Ergebnisse}
  \begin{itemize}
    \item<2-> \color<2>{\clr} Anders als die einfache
      Computersimulation „echtes“ Kippelement
    \item<3-> \color<3>{\clr} Daher Potenzial, Effekte realer
...
\begin{frame}
  ...
  \begin{itemize}
    \item<2-> \color<2>{\clr} Im Modell
      \begin{itemize}
        \item<3-> \color<3>{\clr} Am Zweigpunkt der Hysteresekurve
      \end{itemize}
    \item<4-> \color<4>{\clr} In Realität, Beispiel Wippe
      \begin{itemize}
        \item<5-> \color<5>{\clr} Deutlich früher
        \item<6-> \color<6>{\clr} Zusatzherausforderung beschleunigte Kugel
        \item<7-> \color<7>{\clr} Rollrichtung der Kugel umkehren
        \item<8-> \color<8>{\clr} Ist-Geschwindigkeit der Kugel einrechnen\\
...
\begin{frame}
  ...
  \begin{itemize}
    \item<2-> \color<2>{\clr} Zustand 1: Schneedecke alles bedeckend
    \item<3->[] \color<3>{\clr} \imply{} Schnee reflektiert Licht
    \item<4->[] \color<4>{\clr} \imply{} Geringe Erwärmung
    \item<5->[] \color<5>{\clr} \imply{} Neuschnee bleibt liegen
      {\impliedBy} Mitkopplung!
    \item<6-> \color<6>{\clr} Zustand 2: Schneedecke komplett
      geschmolzen
    \item<7->[] \color<7>{\clr} \imply{} Dunkler Boden absorbiert
      Licht
    \item<8->[] \color<8>{\clr} \imply{} Starke Erwärmung
  ...
  \begin{itemize}
    ...
    \item<5-> \color<5>{\clr} Größe der Hysterese abhängig von
      ...
      \item<7-> \color<7>{\clr} Ggf. schwer zu bestimmen /
        abzuschätzen
      \end{itemize}
    \item<8-> \color<8>{\clr} Hysterese ggf. schon im Zustandsübergang
      voll wirksam
...
\begin{frame}[t]
  ...
  \begin{itemize}
    ...
    \item<10-> \color<10>{\clr} Ggf. zum Anlass nehmen für weitere
      Untersuchungen
  \end{itemize}
\end{frame}

\begin{frame}[t]
  \frametitle{Hysterese: Offene Fragen I}
  Für konkretes reales Kippelement
  \begin{itemize}
    \item<2-> \color<2>{\clr} Wie groß ist die Hysterese (z.B. Schneeflächen)?
      \begin{itemize}
...
\begin{frame}[t]
  \frametitle{Hysterese: Offene Fragen II}
  Für konkretes reales Kippelement
  \begin{itemize}

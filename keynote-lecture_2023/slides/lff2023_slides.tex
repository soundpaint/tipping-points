...
\begin{frame}[t]{Kipppunkt}
  ...
  \tikz[remember picture, overlay] {
    \node[shift={(0.50cm, 0.50cm)}] at (current page.south west) (t2) {
      \begin{tikzpicture}[remember picture, overlay]
        \draw<6-8>[thick,->](0.0, 0.5) -- (10.5, 0.5) node[anchor=north
          west] {\hspace{-3.3cm}\scriptsize Temperatur};
      \end{tikzpicture}
    }
  }
  \tikz[remember picture, overlay] {
    \node[shift={(0.50cm, 0.50cm)}] at (current page.south west) (t3) {
      \begin{tikzpicture}[remember picture, overlay]
        \draw<6-8>[thick,->](0.5, 0.0) -- (0.5, 2.5) node[anchor=south
    ...
    \node[shift={(0.50cm, 0.50cm)}] at (current page.south west) (t2) {
      \begin{tikzpicture}[remember picture, overlay]
        \draw<9>[thick,->](0.0, 0.5) -- (10.5, 0.5) node[anchor=north
          west] {\hspace{-3.3cm}\scriptsize Lage des Schwerpunktes};
      \end{tikzpicture}
    }
  }
  \tikz[remember picture, overlay] {
  ...
  \item<5-> \color<5>{\clr} Sättigung: Begrenzung der Ausgangsgröße
    \begin{itemize}
    \item<6-> \color<6>{\clr} Beispiel: Schneebedeckung in
      niederschlagsreichem Gebiet
      \begin{itemize}
      \item<7-> \color<7>{\clr} maximal 100\% Schneebedeckung
      \item<8-> \color<8>{\clr} minimal 0\% Schneebedeckung
      \end{itemize}
    \item<9-> \color<9>{\clr} Beispiel Wippe: Begrenzte Auslenkung
    \end{itemize}
...
\begin{frame}{Elektronisches Modell: Visualisierung}
  \begin{itemize}
    ...
    \item<4-> \color<4>{\clr} Bildwiederholung für stehendes Bild
      notwendig
    \item<5->[] \color<5>{\clr} {\imply} Kipppelement mit Oszillator
